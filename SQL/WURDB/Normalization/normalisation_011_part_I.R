...
dbuser <- 'pit'; dbpwd <- 'passu'; dbname <- 'pitchipdb'        # Newer PITChip
#dpbuser <- 'pit'; dbpwd <- 'passu'; dbname <- 'phyloarray_pit' # Older PITChip
#dbuser <- 'mit'; dbpwd <- 'passu'; dbname <- 'phyloarray_mit'  # MITChip
#dbuser <- 'mit'; dbpwd <- 'passu'; dbname <- 'chickchipdb'     # ChickChip
con <- dbConnect(drv,username=dbuser,password=dbpwd,dbname=dbname)

# Storage of images
rep.i.path <- "Reproducibility_images/"
spa.i.path <- "Spatial_normalisation_images/"
...
source.file <- "source4normstep2.R"


## Make connection

# THIS PART DOES NOT WORK. I DO NOT KNOW HOW TO CALL ODBC Driver.
#dbuser <- 'pit'; dbpwd <- 'passu'; dbname <- 'pitchipdb'        # Newer PITChip
#dpbuser <- 'pit'; dbpwd <- 'passu'; dbname <- 'phyloarray_pit'  # Older PITChip
#dbuser <- 'mit'; dbpwd <- 'passu'; dbname <- 'phyloarray_mit'   # MITChip
#dbuser <- 'mit'; dbpwd <- 'passu'; dbname <- 'chickchipdb'      # ChickChip
#txt <- paste("SERVER=127.0.0.1; DRIVER=MySQL ODBC 5.5 Driver; DATABASE=", dbname, ";", " UID=", dbuser, "; PWD=", dbpwd, ";", "case=tolower", sep = "")
#txt <- paste("SERVER=127.0.0.1; DRIVER=MySQL; DATABASE=", dbname, ";", " UID=", dbuser, "; PWD=", dbpwd, ";", "case=tolower", sep = "")
txt <- paste("SERVER=localhost; DRIVER=MySQL ODBC 5.1 Driver; DATABASE=", dbname, ";", " UID=", dbuser, "; PWD=", dbpwd, ";", "case=tolower", sep = "")
con <- odbcDriverConnect(connection=txt)
#con <- odbcDriverConnect(connection=paste("SERVER=127.0.0.1; DRIVER=MySQL ODBC Driver; DATABASE=", dbname, ";", " UID=", dbuser, "; PWD=", dbpwd, ";", "case=tolower", sep = ""))
...
viewnorm <- function() {
	...
	tkwm.geometry(gg, "1900x430+1+600")
